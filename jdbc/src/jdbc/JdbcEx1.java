 ...
 * 1. 드라이버 파일(mariadb-java-client-2.4.2.jar) classpath 연결. 사용도구 설정
 * 2. JDBC 관련 패키지 : java.sql.* (java.sql 패키지의 모든 클래스를 등록)
 * 3. 드라이버 클래스를 로드하기
 * 4. db와 Java를 연결하는 객체 생성하기
 * 5. SQL 명령문을 db에 전달하기 위한 객체 생성하기
 * 6. Statement 객체를 이용하여 sql 구문 실행. 결과얻기
 * 7. 결과조회
 */
//import java.sql.*; // java.sql 패키지의 모든 클래스를 등록
import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.ResultSet;
import java.sql.Statement;
public class JdbcEx1 {
	public static void main(String[] args) throws Exception{ // 예외처리하는것임
		// 3. 드라이버 클래스를 로드하기
		// 문자열로 지정된 클래스를 메모리에 로드.
		Class.forName("org.mariadb.jdbc.Driver");
		// 4. db와 Java를 연결하는 객체를 생성하기
		...
		// 문제 : 교수테이블에서 교수의 번호, 이름, id, 입사일, 급여, 보너스, 학과코드 출력하기
