...
public class Exam1 {
	public static void main(String[] args) {
		...
		//3.컴공 학생의 인원수 출력
		System.out.println("컴공학생 인원수 :" + list.parallelStream().filter(s->s.getMajor().equals("컴공")).count());		
		//4.컴공학생의 목록 출력
		System.out.println("컴공학생 목록");
		list.stream().filter(s->s.getMajor().equals("컴공")).forEach(s->System.out.println(s));		
		//5.컴공학생의 수학총점,영어총점,수학평균,영어평균		
		System.out.println("컴공학생 수학 총점:" + list.stream().filter(s->s.getMajor().equals("컴공")).mapToInt(Student::getMath).sum());
		System.out.println("컴공학생 영어 총점:" + list.stream().filter(s->s.getMajor().equals("컴공")).mapToInt(Student::getEng).sum());
		System.out.println("컴공학생 수학 총점:" + list.stream().filter(s->s.getMajor().equals("컴공")).mapToInt(Student::getMath).average().getAsDouble());
		System.out.println("컴공학생 영어 총점:" + list.stream().filter(s->s.getMajor().equals("컴공")).mapToInt(Student::getEng).average().getAsDouble());
	}
}
